...
// A simple shimmer placeholder – darker greys so you actually notice them.
const Shimmer = ({ style }) => {
...
// A little fade‐in wrapper around RN.Image
...
// Wraps each image with a spinner overlay until loaded
...
const getImageStyle = (i, total) => {
  ...
  if (total === 3 && i === 0) return { ...base, width: '100%', aspectRatio: 16 / 9 }
...
export default function ImageGenerationScreen({ navigation }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await generateImage(apiKey, modelToUse, prompt.trim(), numImages)
      ...
      return (
        ...
        >
          <Text
            style={[
              styles.optionText,
              active === val && styles.optionTextActive,
            ]}
          >
            {label}
          </Text>
